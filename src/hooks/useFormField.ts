...
export default function useFormField(props: FormFieldProps): FormFieldData {
    ...
    value.length < props.validation.minLength
        ...
        error = (
      texts?.error_min_chars || defaultTheme.text.error_min_chars
    )?.replace('{}', props.validation.minLength.toString());
    } else if (
    ...
    value.length > props.validation.maxLength
        ...
        error = (
      texts?.error_max_chars || defaultTheme.text.error_max_chars
    )?.replace('{}', props.validation.maxLength.toString());
    } else if (
        props.validation.pattern &&
    value &&
    !props.validation.pattern.test(value)
    ) {
